package org.xiaobo.uml.graphics.util;
...
public class GraphicsUtil {
	...
	public static void paintPlusArea(Graphics g, Point position,
			Dimension dimension) {
		g.pushState();
		int speed = 2;
		...
		for (int i = 0; i < dimension.height; i++) {
			...
			g.drawLine(position.x, position.y + i,
					position.x + dimension.width, position.y + i);
		}
		g.popState();
	}
}
